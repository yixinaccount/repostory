...
<html>
  <head>
	...
	function selListByName(){
		...
		/* 	alert(username);
			$.post("${basePath }/match/matchApplygoto.do", 
					{ matchId: matchId, username: username } ); 
			*/
			
	/*  	 $.ajax({
				url: '${basePath }/match/matchApplygoto.do?matchId='+matchId+'&&username='+username,
				dataType:'json',
				type:'post',
				cache:false,
				async:true,
		...
		//alert(username);
		location.href = "${basePath }/match/matchApplygoto.do?matchId="+matchId+"&&username="+encodeURI(encodeURI(username));
	/*  $.ajax({
				url: '${basePath }/match/matchApplygoto.do?matchId='+matchId+'&&username='+username,
	...
	function showOpt(val, rec) {
		var str = '';
		if (rec.state == '1') {
			str += '<a href="#" style="color:red" onclick="setState(\''+rec.id+'\', \'0\', \''+rec.username+'\',\''+rec.match_name+'\')">禁用</a>&nbsp;&nbsp;&nbsp;&nbsp;'
		} else {
			str += '<a href="#" style="color:green" onclick="setState(\''+rec.id+'\', \'1\', \''+rec.username+'\',\''+rec.match_name+'\')">启用</a>&nbsp;&nbsp;&nbsp;&nbsp;'
		}
		/* str += '<a href="#" style="color:red" onclick="delUser('+rec.id+', \''+rec.babyname+'\',\''+rec.apply_id+'\')">删除</a>'; */
		return str;
	...
	function setState(id, state, parentname,name) {
		...
		$.messager.confirm('状态', "你确定要" + str +"家长  '"+parentname+"' 报名的  '"+name+"' 比赛 吗？", function(r){
			if (r){
				$.ajax({
					url: '${basePath}/match/stateApply.do?matchAppId='+id+'&state='+state,
					dataType:'json',
					type:'post',
					cache:false,
					success:function(data){
	...
	function openAddDialog() {
		 ...
		 parent.opentabwindow('添加比赛信息', 'match/addMatchtogo.do');
		
		/* window.parent.addTab('添加比赛信息', 'match/addMatchtogo.do'); */
	}
	...
	function delUser(id, name,appId) {
		$.messager.confirm('删除', "你确定要删除'"+name+"' 宝贝报名吗？", function(r){
			if (r){
				$.ajax({
					url: '${basePath}/match/delMatchApp.do?id='+id+'&appId='+appId,
					dataType:'json',
					type:'post',
					cache:false,
					success:function(data){
...
<body>
<input value="${matchId}" type="hidden" name="matchId">

<table id="main" class="easyui-datagrid lines-bottom" title="比赛报名列表" style="width:100%"
	...
	<thead>
			<thead data-options="frozen:true">
				<tr>
					...
					<th data-options="field:'birthdayFormat'">出生日期</th>
...
<div id="tb" style="padding:2px 5px;">
	...
	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	<a href="#" class="easyui-linkbutton" data-options="iconCls:'icon-add'" onclick="back()">返回比赛列表</a>
